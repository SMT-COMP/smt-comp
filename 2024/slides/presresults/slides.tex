...
\begin{frame}[fragile]
  \frametitle{SMT-COMP 2024: a year of changes}
  \begin{itemize}
    \item Switch from StarExec to BenchExec
    \begin{itemize}
	    \item A big thanks to Dirk Beyer and Philipp Wendler for offering us to use the VerifierCloud and their support   
        \item A big thanks to Aaron Stump for his StarExec support over the years
    \end{itemize}
    \item New submission process (pull requests)
    \item CI support for submissions
    \item Merge of all competition scripts into one tool
    \begin{itemize}
        \item enhancing their speed by orders of magnitude
    \end{itemize}
    \item New AWS team taking care of the cloud and parallel tracks
    \item Proof track discontinued (for now)
  \end{itemize}
\end{frame}
