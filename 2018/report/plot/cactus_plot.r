...
colors=c("deepskyblue4","red2","orange","chartreuse3","dodgerblue1","dodgerblue3",
...
ltypes=c(5,1,6,4,3,1,5,6,4,3,1,5,6,4,3,1,5,6,4,3,1,5,6)
lwidth=2
nfiles=(length(args)-2)/2
...
if (nfiles >= 2)
{
  for (i in 2:nfiles)
  {
    ...
    xlegend[i] <- as.expression(bquote(bold(.(args[nfiles+i+2]))))
    xcol[i] <- colors[i]
  }
}
