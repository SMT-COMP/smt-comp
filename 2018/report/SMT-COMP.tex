...
instituted in~2005~\cite{TODO}, and is affiliated with the
...
common, community-designed SMT-LIB formats (see
...
SMT-LIB language~\cite{BarFT-RR-17}, such as its floating-point theory
...
usually around mid-April.  The competition chair has ultimate
...
discussions are initiated by a Call for Comments that is sent out to
the SMT-COMP mailing list~\cite{smtcomp-mailinglist} typically in
February or early March.  Publicly, this call is the first harbinger
of a new edition of the SMT Competition.
...
language~\cite{BarFT-RR-17}, a text-based language that defines the
syntax and semantics of SMT solver input and output.  This includes a
language for terms and formulas, and a command language for
interacting with SMT solvers.  For instance,
Figure~\ref{fig:smtlib-example} shows a possible representation of the
example formula from Section~\ref{sec:introduction} as a benchmark in
SMT-LIB syntax.
