...
The SMT Competition was instituted in 2005.  It has been held annually
since then (with the exception of 2013, when an
evaluation~\cite{DBLP:journals/jar/CokSW15} was conducted instead of a
competition).  The main goals of the competition were to spur adoption
of the common, community-designed SMT-LIB formats (see
Section~\ref{sec:smtlib}), and to spark further advances in SMT,
especially for verification~\cite{smtcomp2005}.  These, together with
providing a useful yardstick for users and developers of SMT solvers,
are still its main goals today.  The competition has been successful
in establishing SMT-LIB as the \emph{de facto} standard language for
SMT solvers.  In recent years, the focus in this regard has shifted
towards promoting some of the newer extensions of the SMT-LIB
language, such as a floating-point theory~\cite{TODO} or algebraic
datatypes~\cite{TODO}.
...
organizers, and distributes the work for the next competition.
%
...
Library.  SMT-LIB releases were made on June~1 in 2015, on May~23 in
2016, on June~5 in 2017, and on May~20 in 2018.  Anyone may submit new
benchmarks to the SMT Library, and competitors are permitted to tune
their solvers accordingly.  Therefore it is important that all
benchmarks eligible for the competition are released at least some
time before the final solver submission deadline.  To give the SMT-LIB
maintainers sufficient time for curation, the deadline for new
...
version of the rules is made public on the competition web site
typically around mid-April.  The competition chair has ultimate
responsibility for the rules, but changes are often preceded by
discussions among the organizers or within the SMT community.  Such
discussions are initiated by a Call for Comments that is typically
sent out to the SMT-COMP mailing list~\cite{TODO} in February or early
...
of this penalty is set so that entering a (possibly buggy) solver that
can solve all benchmarks into a division has a positive expected score
if the probability of a soundness bug in the solver is below~$20\,\%$.
...
the number of competitive divisions that a solver has won---was fairly
...
importance, and by also rewarding good performance by solvers other
...
benchmarks in the application and unsat-core track are worth a varying
number of points.  Moreover, in the unsat-core track, the size of the
smallest unsatisfiable core---and hence the number of points
attainable on any given benchmark---may not be known precisely.
