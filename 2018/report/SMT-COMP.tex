...
rewarding imitation of existing solvers rather than true innovation.  In~2016,
...
directly affect the competition results.

Solver responses for such
...
\begin{itemize}[noitemsep]
  \item \makebox[5em][l]{$e\geq 0$} number of erroneous responses (usually~$e = 0$),
  \item \makebox[5em][l]{$0 \leq n \leq N$} number of correct responses (resp.~\emph{reduction} for the \ucoretrack),
...
Non-com-petitive divisions use gray font. % TODO superscript?
...
Table cell colors (other than white) indicate the best competing
solver performance; no cell color indicates that the best solver
...
different winners under this new scheme only. Here we see Alt-Ergo (AUFLIRA in
2018) and STP (QF\_BV in 2016) winning a division for the first time in our
...
constitutes a family had been incorrectly interpreted by the scoring scripts in
2016--2018. The scoring scripts interpreted the bottom most directory
containing benchmarks in a logic as family, while the rules stated that the top
most subdirectory in a logic is to be interpreted as family. After fixing this
misinterpretation we note that only five division winners change---in 2018,
division AUFLIRA should have been won by Vampire instead of CVC4; in 2016,
AUFNIRA should have been won by CVC4 instead of Vampire; in 2016 and 2017,
QF\_LIA should have been won by Yices instead of CVC4; and in 2018, QF\_NRA
should have been won by CVC4 instead of Yices.
Table~\ref{tab:results:total:top} shows the results in the Main Track when
the top most subdirectory in a logic is interpreted as family in reference
to the competition results in Table~\ref{tab:winners-main}, where the bottom
most directory was interpreted as family.
