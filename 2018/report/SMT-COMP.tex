...
unsat-core track, the competition reports sequential and parallel
performance separately.  The former is determined by imposing a
(virtual) CPU time limit of~$T$.  For the application track,
sequential performance is not recorded (because response times to
individual {\tt check-sat} commands are not available).
...
A deeper analysis of the competition results, for example with a view
to improvements in SMT solvers over time, would certainly be
interesting, but goes beyond the immediate mandate of the competition
organizers.  We leave it for future work, noting that all data needed
for such an analysis is publicly available from the competition
website.
