...
given. These tables only lists the result for a division if it has changed
with respect to Table~\ref{tab:winners-main}. A non-competing solver is listed
by itself if there was no competing solver with a positive division score under
the given constraints.
It is interesting to note that we see some new solvers winning divisions that
did not win any division in the main results. For example, veriT solves the
most satisfiable benchmarks in QF\_UFIDL in 2018 and the most unsatisfiable
benchmarks in all four years in UFLRA, and SMTInterpol performs very well on
unsatisfiable benchmarks in QF\_LIA.
