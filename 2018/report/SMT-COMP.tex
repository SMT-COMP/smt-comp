...
\begin{table}
  ...
  \centering
  \begin{tabular}{|l|p{11.7cm}|} %TODO: fix width
    \hline
...
\begin{table}
  ...
  \centering
  \scalebox{.89}{\begin{tabular}{|c|p{2.2cm}|r|r|p{0cm}|c|p{2.2cm}|r|r|}
    \cline{1-4} \cline{6-9}
    \multicolumn{4}{|c|}{2015} & & \multicolumn{4}{c|}{2016} \\ \cline{1-4} \cline{6-9}
    Rank & Solver & Score  & Score  & & Rank & Solver & Score & Score \\
         &        & (sequential) & (parallel) & & & & (sequential) & (parallel) \\ \cline{1-4} \cline{6-9}
      ...
      3. & Yices             & 101.91 & 101.91 & & 3. & veriT  &  75.11 &  75.11 \\ \cline{1-4} \cline{6-9}
  \end{tabular}}
  ...
  \scalebox{.89}{\begin{tabular}{|c|p{2.2cm}|r|r|p{0cm}|c|p{2.2cm}|r|r|}
    \cline{1-4} \cline{6-9}
    \multicolumn{4}{|c|}{2017} & & \multicolumn{4}{c|}{2018} \\ \cline{1-4} \cline{6-9}
    Rank & Solver & Score  & Score  & & Rank & Solver & Score & Score \\
         &        & (sequential) & (parallel) & & & & (sequential) & (parallel) \\ \cline{1-4} \cline{6-9}
         & [Z3]        & 171.99 & 171.99 & & 1. & CVC4        & 211.99 & 211.99 \\
      1. & CVC4        & 161.38 & 161.76 & &    & [Z3]        & 186.19 & 186.19 \\
      2. & Yices2      & 110.63 & 110.63 & & 2. & Yices2      & 115.26 & 115.26 \\
      3. & SMTInterpol &  65.96 &  66.00 & & 3. & SMTInterpol &  65.32 &  65.38 \\ \cline{1-4} \cline{6-9}
  \end{tabular}}
\end{table}
...
\item the solvers that won the QF\_BV division in the main track (see
  Table~\ref{TODO}),
  % TODO (sequential ranking: Boolector, parallel ranking: Minkeyrink-MT);
...
%TODO The latter two medals were won by Yices-2.6.0.
...
After 13 years, the SMT Competition is a well-established event that
garners wide interest from developers and users of SMT solvers.  The
competition continues to contribute to its original goals, including
adoption of the SMT-LIB format and sparking further advances in SMT,
and it provides a yardstick of performance for SMT solvers.
...
the earlier culling of benchmarks was abandoned.  Since the
re-introduction of the unsat-core track in~2016, the competition again
has three separate tracks.  This, together with new logics in the SMT
Library, has brought the total number of competition divisions to 115
in~2018, up from just 40 divisions in~2014~\cite{CDW14}.  The number
of job pairs increased from 339,714 in~2014 to~1,776,062~in 2018, and
the total wall-clock time in~2018 exceeded 7.6~years.  By these
measures, the SMT Competition is one of the largest competitions in
automated reasoning.
...
participants and benchmarks each year, to determine whether the
current time limits remain tenable.  (Otherwise, reducing the time
limits would probably be a better solution than reverting to the
pre-2014 practice of benchmark culling.)

Further development of the competition should be undertaken with care,
keeping in mind that any major changes require not just the
competition organizers, but also participants to adapt.  Immediate
action items for next year's organizers are
\begin{itemize}
...
Organizing the competition is a non-trivial amount of work.  Much of
this happens behind the scenes.  For instance, the competition tools
(such as the benchmark scrambler and trace executor) often require
updating when new divisions are added, or when changes are made to
StarExec.  One could imagine adding further tracks (e.g., for
proof-producing SMT solvers, or for variants such as max-SMT) to the
competition, but the effort required to support such tracks will need
to be balanced against their added value.
...
then be used in the competition---a straightforward way to get solver
developers to tune their solvers accordingly.
