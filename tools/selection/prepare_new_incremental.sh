...
echo "pair id,benchmark,benchmark id,solver,solver id,configuration,configuration id,status,cpu time,wallclock time,memory usage,result,expected" > new_incremental.csv 

pushd $1
find . -name '*smt2' | sed 's/\.\///' | sed 's/\(.*\)/x,\1,x,NEW,x,x,x,timeout (wallclock),5000,5000,0,starexec-unknown,starexec-unknown/g' > smtcomp-tmp 
popd
mv $1/smtcomp-tmp .
cat < smtcomp-tmp >> new_incremental.csv 
rm smtcomp-tmp
