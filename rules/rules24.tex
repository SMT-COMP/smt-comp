...
\begin{itemize}
  ...
  \item {\bf Move from StarExec to BenchExec.}  \emergencystretch.5cm
  The single query, incremental, unsat-core, and model-validation will no longer 
  run on the StarExec platform hosted by the University of Iowa.
  Starting this year, we will run these tracks on the BenchExec cluster 
  (\url{https://github.com/sosy-lab/benchexec}) owned by LMU's Software 
  and Computational Systems Lab (SoSy-Lab \url{https://www.sosy-lab.org/}).
%
  \rationale{The StarExec cluster is in the process of being decommissioned. We picked
             BenchExec as the new platform because it is host to similar competitions
             and the experience collected there is a big help in our migration process.
             Moreover, it is in academic hands, so less likely to change conditions on
             short notice.}

  \item {\bf Submission process.}  \emergencystretch.5cm
  We changed the submission process from a Google form to a push request to the
  SMT-COMP GitHub repository (\url{https://github.com/SMT-COMP/smt-comp.github.io/tree/master/submissions}).
  Detailed instructions can be found in the README file there.
  Moreover, participants are now required to upload their final solver versions as a Zenodo
  (\url{https://zenodo.org/}) submission.
%
  \rationale{The change from Google form to GitHub push request makes it easier for us to
             automatically create scripts for BenchExec. The submission of the final solver
             versions to Zenodo makes it more likely that the competition results stay replicable.}

  \item {\bf Time limits.}  \emergencystretch.5cm
  The time limits for the cloud and parallel tracks are reduced to 2 minutes per problem.
%
  \rationale{The analysis of last years results showed that most problems in these tracks are solved quickly or not at all.}

  \item {\bf Memory limits.}  \emergencystretch.5cm
  The memory limits for the single query, incremental, unsat-core, and model-validation tracks are reduced to 30\,GB.
%
  \rationale{The reasons are hardware limits of the new execution platform.}
  
  \item {\bf Proof track.}  \emergencystretch.5cm
  The previously experimental evaluation proof track will be discontinued.
%
  \rationale{There are three reasons that led to this decision. 
             First, we were unable to find a way of turning the proof track into a competition that would not lead to 
             an unmanageable amount of overhead on the competition organizers or the participants.
             Second, we have collected vast amounts of data in the last two years and thoroughly evaluating it takes time.
             Third, this year we would need to restrict the allowed proof size due to hardware limits of the new execution platform.}
  
  \item {\bf Tools.}  \emergencystretch.5cm
    All the relevant scripts in \url{https://github.com/SMT-COMP/smt-comp} have been combined into one tool called \textbf{smtcomp}
    that is available at 
    \url{https://github.com/SMT-COMP/smt-comp.github.io}.
%
  \rationale{The reason for this new tool is that the sheer number of old scripts and their various locations 
             caused a lot of confusion to many new and several longtime participants.
             The tool also has better run time compared to some of the old scripts because it uses parallelization where possible.
             Moreover, it provides a nicer UI compared to the old scripts.}
\end{itemize}
...
organisers. The final solver version needs to be uploaded as a Zenodo (https://zenodo.org/ <https://zenodo.org/>) submission. The solver is an archive that contains the executable precompiled (statically linked is preferable). It will be executed on a computer that have the same installation than the docker image \texttt{registry.gitlab.com/sosy-lab/benchmarking/competition-scripts/user:latest}.
...
the above URL. For the \cloudtrack{} and \paralleltrack{},
