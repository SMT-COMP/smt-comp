...
\begin{itemize}
  \item {\bf Portfolio Solvers are Forbidden.} \todo{check}
    ...
    \todo{Something about  hybrid solvers?}
  ...
  \item {\bf Limit the Number of Solvers.} \todo{NEW}
    ...
    \rationale{The organizers wishes to keep the total run-time of the
  ...
  \item {\bf Combine stdout and stderr.}
    ...
    \rationale{Due to limitations of starexec, the standard error channel
      and standard output channel are not separated.  The previous year the
...
%    benchmarks beyond \akey{check-sat} commands with unknown status.}
  ...
  \item {\bf Time Limit.} The time limit per solver/benchmark pair is
    ...
    reduced to 20 minutes (down from 40 minutes in earlier years) \todo{rationale}.
    For the \challtrack, 12 hours seem to be a reasonable compromise for the
    expected number of participants and benchmarks submitted to this track.}

  \item {\bf Benchmark Selection.} \todo{update}
    Since 2015, the competition evaluated all
  ...
  \item {\bf Competition-Wide Recognitions.}
    This year, we will replace the previous notion of competi\-tion-wide scoring
    ...
    \rationale{Competi\-tion-wide scoring was introduced as a metric to
    compare solver performance across all divisions. It is, by definition,
    biased towards solvers that enter a large number of divisions.}
...
\header{Multiple versions.}\todo{check}
%
...
\header{Attendance.}\todo{fix this!}
%
...
pair.\todo{we said 20 minutes, right?}
...
other commands.
Solver must not write anything to the standard error channel. \todo{okay?}
...
\begin{enumerate}
  ...
  \item INVALID\hskip .5em for
    \begin{itemize}[noitemsep,topsep=0pt]
      ...
      \item partial models,
      \item malformed models (except if this due to reaching the time limit
        while printing),
...
seeding values. \todo{Should we provide more randomness by multiplying it with 100?  It has two or three trailing digits}  This helps provide transparency, by guaranteeing that
...
Jochen Hoenicke is part of the developing teams of the SMT solver
