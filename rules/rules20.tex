...
SMT-COMP~2020.
%
\ah{As in previous years, we have revised the rules slightly.  Some rule
...
\item \ah{{\bf Model validation track for QF\_BV is not
        experimental.}
        Last year's competition introduced the experimental model
        validation track for QF\_BV.  This year this will be a standard
        track.}

        \rationale{There were no issues with the track, so there is no
        reason to keep it experimental.}
\end{itemize}
  ...
  \begin{itemize}
    \item \emph{\maintrack.} All benchmarks that were solved by all
      solvers (including non-competitive solvers) in less than one second in
          the corresponding track in \hb{previous years}.%  2018 \ah{or 2019}.
