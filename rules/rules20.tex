...
\item[May~25] Deadline for final versions of solvers, including
  system descriptions.
\item[May~26] Opening value of NYSE Composite Index used to compute
...
\begin{itemize}
  \item {\bf Portfolio Solvers are Forbidden.}
    This year we explicitly forbid the submission of so-called
    portfolio solvers as competition entries.  We acknowledge that
    judging what constitutes a portfolio SMT solver is difficult.
    However, we are not the first in history to face a similar
    challenge, and plan to apply an equally historical approach where
    we write down a ``law'', act as its ``judges'', and have a few
    ``precedents''.
    ...
    \rationale{The goal of the competition is to evaluate and improve
      the state of the art of SMT solving. By just combining existing
      solvers and running them in parallel it is possible to win the
      competition without contributing anything new.}

  \item {\bf Limit the Number of Solvers.}
  ...
  \item {\bf Combine stdout and stderr.}
    This year, output to the standard error channel is not ignored, but
    treated the same as output to the standard output channel.
    Solvers should not write anything to the standard error channel.
%
    \rationale{Starexec does not distinguish between the standard error
      and output channels.  In the previous year, the
      organizers had to work around the limitation by wrapping the submitted
      solvers in a script that ignored stderr.  By changing the rule we avoid
      that the organizers have to change the submitted solvers.}

%  \item {\bf The \mvaltrack is extended to other logics.}
%    This year, we introduce a new experimental \mvaltrack for the logics QF\_IDL, QF\_RDL, QF\_LIA, QF\_LRA, QF\_LIRA.
...
%    years,  we hope for a high number of participants in this track.  }
  ...
  \item {\bf The \challtrack will not run this year.}

  \rationale{Since there were no new benchmarks submitted to SMT-LIB intended
    for the SMT-COMP \challtrack, we will not run the track in 2020, as
  ...
  model validation track for QF\_BV.  This year this will be a standard
  track. Moreover, the divisions QF\_IDL, QF\_RDL, QF\_LIA, QF\_LRA, QF\_LIRA
  will be included and will be experimental.

  \rationale{There were no issues with the track for QF\_BV, so there is no
...
will be accepted until the end of {\bf May~25, 2020} anywhere on earth.
...
\begin{enumerate}
  ...
  \item INVALID\hskip .5em for
    \begin{itemize}[noitemsep,topsep=0pt]
      \item an \texttt{unsat} solver response to \akey{check-sat},
      \item partial models,
      \item malformed models (except if this is due to reaching the time limit
        while printing),
      \item models that do not satisfy the input problem; and
    \end{itemize}
  \item UNKNOWN\hskip .5em for
    \begin{itemize}[noitemsep,topsep=0pt]
      \item no solver output (no response to either both commands or
        \akey{get-model}),
      \item an \texttt{unknown} response to \akey{check-sat}.
    \end{itemize}
\end{enumerate}
