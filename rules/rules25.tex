...
\begin{itemize}
  ...
  \item {\bf Derived tools.} \emergencystretch.5cm Submitters of a derived tool should
...
The final solver version needs to be uploaded as a Zenodo (\url{https://zenodo.org/}) submission.
In the case of derived tools, the Zenodo submission should include both the derived tool and the base tool.
The solver is an archive that contains the executable precompiled (statically linked is preferable).
It will be executed on a computer that have the same installation as the following docker image
\begin{center}
  \small
...
\emph{and the tracks and divisions which it enters}, by submitting a properly configured
...
(\url{https://zenodo.org/}) submission (together with the base solver in the case of a
derived solver).
...
However, updates to existing entrants via pull requests or
will be accepted until the end of {\bf June~27, 2024} anywhere on earth.
...
competitions. Starting from 2019, the submitted solvers are included in the
archive to allow future reproduction of the competition results.
%
...
20 minutes per solver/benchmark pair. The time limit for checking
unsatisfiable cores will be 20 minutes of wall-clock time per solver.
