...
\dd{Fill when dates are defined.}
\begin{description}
...
%
The principal changes from the previous competition rules are the following:\dd{To be reviewed}
\begin{itemize}
...
more sub-solvers\dd{What is a sub-solver?}, developed by different groups of authors, on the
same component \dd{What is a component?} or abstraction of the input problem.  For example, a
...
subsolvers from different groups of authors is not\dd{These first two sentences seem contradictory...}.
...
However, it is not allowed to call more than one different SMT solver \dd{"more than one different" or "more than one"?} to solve a problem over the input logic.
...
organisers\dd{To be updated}. The final solver version needs to be uploaded as a Zenodo (https://zenodo.org/ <https://zenodo.org/>) submission.
The solver is an archive that contains the executable precompiled (statically linked is preferable).
It will be executed on a computer that have the same installation as the docker image \texttt{registry.gitlab.com/sosy-lab/benchmarking/competition-scripts/user:latest}.
...
\end{center}
The template for the JSON submission file is available under this URL, in the \texttt{template} directory.
The JSON file shall be submitted through a pull request titled \texttt{submission: <identifier>},
where \texttt{<identifier>} shall identify the submitted solver.
...
\dd{Explain how to fill the JSON file here:}
...
\begin{itemize}
  \item \textbf{smtcomp.} This tool combines the functionality of most of the
    ...
    \url{https://github.com/SMT-COMP/smt-comp.github.io}. Some of its functionalities include:
    \begin{itemize}
    ...
    \end{itemize}
    For a full list of the capabilities of the smtcomp tool and an explanation of all important commands
