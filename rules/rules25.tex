...
As in previous years, we have revised the rules slightly. Most changes are due to our move from StarExec to BenchExec.
%
...
%
  ...
  \item {\bf Time limits.} \emergencystretch.5cm The time limit to verify the unsat cores
  is increased.
%
  \rationale{In the unsat-core track, points shall be awarded to a solver for all correct produced unsat cores.}

  \item {\bf Unsat cores.} \emergencystretch.5cm No point is given for an unsat core such that no solver is capable to
  verify it.
%
  \rationale{In the unsat-core track, points shall be awarded to a solver only for correct produced unsat cores.}
