...
SMT-COMP 2025 will have seven tracks: the \maintrack (before 2019: Main Track),
the \inctrack (before 2019: Application Track), the \ucoretrack, the \mvaltrack,
and the \paralleltrack{} and the \cloudtrack{} \dd{maybe not sponsored actually}(both sponsered by Amazon Web
Services).
%
...
a pull request to the SMT-COMP GitHub repository (\url{https://github.com/SMT-COMP/smt-comp.github.io/tree/master/submissions}), or, for
\paralleltrack{} and \cloudtrack{}, otherwise communicated to the
organisers\dd{To be updated}. The final solver version needs to be uploaded as a Zenodo (\url{https://zenodo.org/}) submission.
...
\dd{change this probably} For the \cloudtrack{} and \paralleltrack{},
...
(\url{https://zenodo.org/}) submission.
...
SMT-COMP entrants must be submitted via a pull request as explained above and,
in the case of the \paralleltrack{} and \cloudtrack{},
communicated separately to the organisers via email until the end of
{\bf May~27, 2024}\dd{Update according to new infrastructure.} anywhere on earth.
...
but is anticipated to be around 5 minutes of wall-clock time per solver\dd{update if increased}.
