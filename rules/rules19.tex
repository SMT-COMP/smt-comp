...
the incremental track (previously: Application Track), the \ucoretrack,
...
\begin{itemize}
  ...
  \item {\bf New \mvaltrack for QF\_BV.}
    This year, we introduce a new experimental \mvaltrack for logic QF\_BV.
  ...
  \item {\bf New \challtrack.}
    This year, we further introduce a new \challtrack.  This track will contain
    ...
    the \maintrack'').  }
    \todo{how do we provide information about which benchmarks and which logics?}
  ...
  \item {\bf Competition-Wide Scores/Winners.} \todo{}
...
\header{SMT Solver.}
%
A solver that can enter SMT-COMP is a tool that allows to
determine (un)satisfiability of benchmarks from the SMT-LIB benchmark library
(\url{http://www.smt-lib.org/benchmarks.shtml}).

\header{Wrapper Tool.}
%
A \emph{wrapper tool} is defined as any solver that calls one or more SMT
solvers (the \emph{wrapped solvers}) of which the author of the wrapper tool is
not an author.  Its system description \textbf{must} explicitly acknowledge any
...
extends} an SMT solver (the \emph{base solver}) not written by the authors of
the derived tool.  Its system description \textbf{must} explicitly acknowledge
the solver it is based on and extends.  It \emph{should} further make clear
technical innovations by which the wrapper tool expects to improve on the
wrapped solvers.  A derived tool should follow the \emph{naming convention}
...
The competition website (\url{www.smtcomp.org}) will be used as the main form of communication for the competition. The website will be used to post updates, link to these rules and other relevant information (e.g. the benchmarks), and to announce the results. We also use the website to archive previous competitions. Starting from 2019 we will include the submitted solvers in this archive to allow reproduction of the competition results in the future.
%

\header{Tools.} 
The competition uses a number of its own tools/scripts to run the competition. We briefly describe these tools. Unless stated otherwise the tools are found at \url{https://github.com/SMT-COMP/smt-comp/tree/master/tools}. {\color{red} Some tools have not been released yet.}
\begin{itemize}
	\item \textbf{Benchmark Selection.} This script implements the benchmark selection policy described on page~\pageref{benchmark-selection}. It takes a seed for the random benchmark selection. The same seed is used for all tools requiring randomisation.
	\item \textbf{Scramber.} This tool is used to scramble benchmarks during the competition to ensure that tools do not rely on syntactic features to identify benchmarks. The scramber can be found at \url{https://github.com/SMT-COMP/scrambler}.
	\item \textbf{Trace Executor.} This tool is used in the interactive track to emulate an on-line interaction between an SMT solver and a client application and is available at \url{https://github.com/SMT-COMP/trace-executor}
	\item \textbf{Postprocessors.} These are used by StarExec to translate the output of tools to the format required for scoring. The postprocessors (per track) are available at \url{https://github.com/SMT-COMP/postprocessors}.
	\item \textbf{Scoring.} This script implements the scoring computation described on in Section~\ref{sec:scoring}. It also includes the scoring computations used in previous competitions (since 2015).
\end{itemize}

\header{Input and output.}
%
In the \maintrack, \mvaltrack, \ucoretrack and \challtrack, a participating
solver must read a single benchmark script, whose filename is presented as the
solver's first command-line argument.  In the \inctrack, a trace
executor will send commands from a benchmark script to the solver's standard
input channel.
...
\item
  \begin{enumerate}
  \item In the \maintrack and \ucoretrack, there may be a single
  ...
  \end{enumerate}
\item In the \ucoretrack, there is a single \akey{(set-option
...
\item
  \begin{enumerate}
    \item In all tracks except the \ucoretrack,  named terms (i.e., terms
      with the \akey{:named} attribute) are not used.
    \item In the \ucoretrack, top-level assertions may be named.
  \end{enumerate}
\item
  \begin{enumerate}
  \item In all tracks except the \inctrack, there is exactly one
  ...
  \end{enumerate}
\item In the \ucoretrack, the \akey{check-sat} command (which is
  always issued in an unsatisfiable context) is followed by a single
  \akey{get-unsat-core} command.
\item In the \mvaltrack, the \akey{check-sat} command (which is
...
\header{Time and memory limits.} \todo{Aina: Move all time and memory limits to the individual sections and forward reference from here}
%
...
The \maintrack competition will consist of selected benchmarks in each
...
The \maintrack competition uses a StarExec post-processor (named
...
\todo{time limit}
The \challtrack will include both incremental and single query benchmarks and it will follow the same rules as the \inctrack and  \maintrack, respectively with two exceptions: benchmark selection and the time limit. The track will run on challenging industrial benchmarks provided by the community.
...
\subsection{Competition-wide scoring (\maintrack)}
...
We define a competition-wide metric for the \maintrack, separately for
