...
\begin{itemize}
  ...
  \item {\bf New \challtrack.}
    ...
    the \maintrack'').  } See ~\ref{sec:exec:industry-challenge} for the list of
    benchmark families included in \challtrack for this year.
...
solvers (the \emph{wrapped solvers}) that do not have the same author as the
wrapper tool itself. Its system description \textbf{must} explicitly acknowledge any
...
\todo{time limit}
The \challtrack will include both incremental and single query benchmarks and it will follow the same rules as the \inctrack and  \maintrack, respectively with two exceptions: benchmark selection and the time limit. The track will run on challenging industrial benchmarks provided by the community. This year, the \challtrack will include all the benchmarks from the \lh{TODO families} families, in the \lh{TODO logics}.
