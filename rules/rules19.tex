...
\header{Benchmark Selection.}
This track will run on all \emph{unsatisfiable} benchmarks from the \maintrack,
modified to use named top-level assertions of the form \akey{(assert (! t
:named f ))}.
\todo{what if removing all benchmarks with one assertion doesn't leave enough benchmarks in a division?}
...
\emph{satisfiable} benchmarks from this division in the \maintrack.
...
to this track by their submitters. The \ucoretrack will use all unsatisfiable
\maintrack benchmarks, modified to use named top-level assertions.  The
\mvaltrack will use all satisfiable \maintrack benchmarks from the QF\_BV
division.
...
\subsection{Division scoring \todo{update}}
