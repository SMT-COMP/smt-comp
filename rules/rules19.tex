...
\begin{itemize}
  ...
  \item {\bf Benchmark Selection.} Since 2015, the competition evaluated all
    solvers on all eligible benchmarks in SMT-LIB.  This year, we will use an
    alternative benchmark selection scheme that selects a subset of the
    eligible benchmarks. The upper bound for the size of each division,
    however, is not fixed, but depends on the size of the corresponding logic.
    Prior to this selection, we will remove all benchmarks in a division that
    were solved by all solvers (including non-competing solvers) in this
    division in under a second during last year's competition.
    \rationale{Evaluating solvers on all eligible benchmarks in SMT-LIB makes
    results more predictable and seems to be more of an evaluation than a
    competition.  In the Main track of last year's competition, 78\% of the
    258,741 benchmarks were solved by all supported solvers within the time
    limit (71\% within 1 second). In 7 (out of 46) logics, over 99\% of
    benchmarks were solved by all solvers.  Removing `easy' (or at least
    `unsurprising') benchmarks attempts to shift the focus towards challenging
    benchmarks, in particular since we now use an alternative benchmark
    selection scheme.  It can be argued that the size of a logic in SMT-LIB can
    be seen as an indicator of its relevance.  We thus do not use a fixed upper
    bound for the size of a division in order to reflect the suggested
  ...
  \item {\bf Do Not Run Non-Competitive Divisions.} \todo{}
...
%
\todo{add minimum requirement for authorship?}
...
%
\todo{Giles: update}
\todo{reword? "one or more SMT solvers of which the author of the wrapper tool
is not an author"}
...
\header{Tools} \todo{link to scrambler, trace executor, benchmark selection script, postprocessors}

\header{Input and output.}
%
In the \maintrack and \ucoretrack, a participating solver must read a
...
\header{Benchmark selection.} \todo{Antti covers this section}
%
The competition will use a large subset of SMT-LIB benchmarks, with a
preference to the new benchmarks.  In all tracks the following process
will be used for culling the benchmark pool.
\begin{enumerate}
...
\item \emph{Cap the number of instances in a division.} The competition
  ...
  \item If a division contains between 300 and 600 instances, a
        random selection of 300 instances from the set will be used.
  \item If a division contains more than 600 instances, 50\% of
        the benchmarks will be randomly selected.
  \end{enumerate}
\item \emph{\ucoretrack{}.} In addition, for the \ucoretrack{}, all
  benchmarks with a single assertion will be removed.
\end{enumerate}
%
The culling script will be publicly available and use the same random
seed as the rest of the competition, and the set of benchmarks selected
for the competition will be published when the competition begins.
