...
\header{Result.}
The result of a solver is considered \emph{erroneous} if the response to the
...
Note that an \texttt{unsat} response to \akey{check-sat} but no response to
\akey{get-model} is considered as a special case of the last case
...
\begin{enumerate}
  ...
  \item INVALID\hskip .5em for
    \begin{itemize}[noitemsep,topsep=0pt]
      ...
      \item a \texttt{sat} response to \akey{check-sat} without a response to
        \akey{get-model},
      \item partial models,
      \item malformed models,
      \item models that do not satisfy the input problem; and
    \end{itemize}
  \item UNKNOWN\hskip .5em for
    \begin{itemize}[noitemsep,topsep=0pt]
      \item no solver output (no response to both commands),
