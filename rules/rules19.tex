...
\begin{enumerate}
  \item
    \begin{enumerate}
      \item In the \textbf{\maintrack} and \textbf{\ucoretrack}, there may be a single
    ...
    \end{enumerate}
  \item A (single) \akey{set-logic} command is the \emph{first} command after
    any \akey{set-option} commands.
  \item It may contain any number of \akey{set-info} commands.
  \item It may contain any number of \akey{declare-sort} and
    \akey{define-sort} commands.  All sorts declared or defined with
    these commands must have zero arity.
  \item It may contain any number of \akey{declare-fun} and \akey{define-fun}
    commands.
  \item If the logic features algebraic datatypes, the benchmark file may
    contain any number of \akey{declare-datatype(s)} commands.
  \item It may contain any number of \akey{assert} commands.  All formulas in
    the file belong in the declared logic, with any free symbols declared in
    the file.
  \item
    \begin{enumerate}
  ...
  \item
    ...
    \end{enumerate}
  \item In the \textbf{\ucoretrack}, the \akey{check-sat} command (which is
    always issued in an unsatisfiable context) is followed by a single
    \akey{get-unsat-core} command.
  \item In the \textbf{\mvaltrack}, the \akey{check-sat} command (which is
    always issued in a satisfiable context) is followed by a single
    \akey{get-model} command.
  \item It may \emph{optionally} contain an \akey{exit} command as its
