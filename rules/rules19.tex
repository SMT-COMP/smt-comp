...
\item[June~2] Deadline for final versions of solvers, including
  system descriptions.
\item[\an{TODO}] Opening value of NYSE Composite Index used to complete
...
SMT-COMP 2019 will have five tracks: the single problem track (formerly known
as Single Problem Track), the incremental track (formerly known as application
track), the unsat-core track, the (new) industry challenge track, and the (new)
...
\an{Add information that these solver versions will be made available
...
\header{Input and output. \rem{TODO: update}}
%
...
\item
  \begin{enumerate}
    \item In the main and incremental track, named terms (i.e., terms
...
pair in the single problem, incremental and unsat core tracks, \rem{TODO:
XX}~minutes in the industry challenge track, and \rem{TODO: XX}~minutes in the
model validation track \an{TODO: Need to elaborate how time is split? Do we
...
\subsection{Single Problem Track (Previously: Main Track)}
\label{sec:exec:single}

The single problem track competition will consist of selected benchmarks in each
...
The single problem track competition uses a StarExec post-processor (named
...
\rem{TODO: Update accordingly: Do we run only up to the first unknown status
check-sat or further?}
...
benchmark in the incremental track represents a realistic
...
same as for the single problem track (see Section~\ref{sec:exec:single}).
...
use a selection of SMT solvers that participate in the single problem track of
...
in the single problem track for this division.  The unsatisfiability of a
...
\subsection{Industry challenge track}
\label{sec:exec:industry-challenge}

The industry challenge track will follow the same rules as the single problem track,
...
\subsection{Model Validation Track (experimental)}
\label{sec:exec:model}
The model validation track will evaluate the solver's ability to produce models
for satisfiable problems. This track will run on all QF\_BV satisfiable benchmarks
from the single problem track.
...
\an{TODO: We do not run non-competitive divisions.}


\header{Benchmark sources.}
%
Benchmarks for each division will be drawn from the SMT-LIB benchmark
library.  The single problem track will use a subset of all
...
will use unsatisfiable single problem track benchmarks, modified to use named
...
\header{Benchmark demographics. \rem{TODO: check}}
%
...
\header{Benchmark selection. \rem{TODO: update}}
%
...
\section{Scoring \rem{TODO: update}}
\label{sec:scoring}
...
results,\footnote{Here, $N$ is the number of \akey{check-sat} commands
  in the benchmark.  Recall that single problem track benchmarks have just one
...
\header{Single Problem Track.} More specifically, for the single problem track,
we have
%
\begin{itemize}
...
\subsection{Sequential Performance (Single Problem Track)}
...
best parallel solvers in all competitive single problem track divisions.
\fi
...
24-second timeout.
\rem{TODO: 24-seconds is only a score, not a track}

\header{Single Problem Track: Removal of Disagreements.}
%
Before division scores are computed for the Single Problem Track, benchmarks
...
for a division if its parallel performance (Section~\ref{sec:benchmark-scoring})
...
For Single Problem Track and unsat-core track divisions, we will separately
...
\subsection{Competition-wide scoring (Single Problem Track)}
\label{sec:competition-wide}

\rem{TODO: Replace notion of competition-wide scoring with a notion of ranking
based on distance between first and second (based on scoring, and maybe a second
based on VBS marginal contribution)}

We define a competition-wide metric for the Single Problem Track, separately for
...
\subsection{Other Recognitions \rem{TODO: Update}}
...
\item the solver that wins the QF\_BV division (separately for
  parallel and sequential performance) in the Single Problem Track.
\end{itemize}
