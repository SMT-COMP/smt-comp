...
\begin{itemize}
\item The organizers plan to recognize additional special
  contributions.  These might include:
  \begin{itemize}
  \item Best application track solver (awarded based on a
    competition-wide scoring for the application track)
  \item Best unsat-core track solver (awarded based on a
    competition-wide scoring for the unsat-core track)
  \item Best complementary solver (awarded to the solver that performs
    best when all benchmarks solved by the winning solver are removed)
  \item Most unique solver (awarded to the solver that performs best
    on benchmarks solved by only one solver)
  \item The Prophet (awarded to the solver that performs best on
    benchmarks with unknown status)
  \item The 10 Second Wonder (awarded to the solver that would have
    won if the time limit was 10 seconds)
  \end{itemize}
  The details are still being discussed and subject to change.
...
\item In 2017, the competition featured experimental divisions for
  benchmarks that use algebraic datatypes.  Provided the competition
  tools can be enhanced in time to support datatypes, these divisions
  may become regular (non-experimental) in 2018. \emph{Rationale:}
  Algebraic datatypes are specified in the latest official release
  (Version~2.6) of the SMT-LIB standard.
  
\item The competition may feature experimental divisions for
  benchmarks that use strings, and for benchmarks that use
  transcendental functions.  \emph{Rationale:} Corresponding theories
  and benchmarks are expected to be added to SMT-LIB in the near
  future.
\end{itemize}
