...
\item[June~10] Deadline for final versions of solvers, including
  system descriptions.
\item[\rem{June~20 TODO update}] Opening value of NYSE Composite Index used to complete
...
\rem{Aina: Is this up to date? We do use weighting to determine a score that
determines the winner, right? So, it is actually not the number of solved
benchmarks that determines the winner.}
...
following:
\rem{Aina: All of this bullets are not new in 2018 anymore, maybe move them
elsewhere? This information should not be lost.}
\begin{itemize}
...
\end{itemize}
\rem{Aina: Are we going to introduce an experimental strings divisions?}
...
\rem{Aina: Why are there no restrictions as to who can submit which solver?
Shouldn't we maybe introduce the requirement that only authors can submit
solvers? Else someone could, e.g., submit CVC4 or Z3 as their solver.}

\header{System description.}
%
As part of a submission, SMT-COMP entrants are encouraged to provide a
short (1--2 pages) description of the system.
\rem{Aina: I would actually make the submission of the system description a
requirement, if possible. I would also require that if entrants are based
on an already existing solver, this must be mentioned in the system description
and appropriate acknowledgement is due, same as with wrapper tools.}
...
\rem{Aina: Again, I'd like to make the submission of system descriptions
mandatory. SAT competition does that as well, btw.}
...
scores.
\rem{Aina: I strongly suggest that we make disagreements visible.
I would add a table to the bottom of the division results page, where
we list the benchmarks we removed due to disagreements and which results
which solvers reported. Silently dropping benchmarks on which solvers
disagree is potentially unfair and dangerous, since people who want to use
a solver, tend to only
look at the SMT-COMP results and pick a solver based on this results.
They will certainly not go through the competition results on StarExec,
in particular since this is quite tedious.
For users it is interesting (and might be actually crucial) to know
if a solver disagreed with other solvers.
The same is true for solver developers, since they certainly want to know
if their solver disagreed with other solvers on a certain benchmark.
They usually run preliminary tests with existing solvers beforehand,
but not all solvers are available in the newest version.
Not to mention new solvers.}
...
\rem{Aina: Do we already know how many medals FLOC is going to sponsor?
Clark offered to pay for printing proper and beautiful certificates for each
division if we don't have funds for that.
As for medals, it might be a little late to get sponsors (and the funds
from SMT-LIB went to the travel fund).}

\rem{Aina: Depending on the number of medals we get from FLOC we should
decide on how to categorize for prizes. Some ideas:
\begin{itemize}
  \item Competition-wide, main track, best solver
  \item Quantified divisions, best solver
  \item Either QF\_BV best solver, or QF\_BV + QF\_ABV + QF\_AUFBV + QF\_UFBV
  \item Comptetition-wide, application track, best solver
  \item Comptetition-wide, unsat-core track, best solver
  \item Best newcomer
\end{itemize}
Floating-point is currently only officially supported by Z3,
inofficially also by CVC4, if we expect more than 2 entrants, then FP logics
...
producing the Vampire system\rem{TODO for Giles: cite Vampire}~\cite{vampire}.
