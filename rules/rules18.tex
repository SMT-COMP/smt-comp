...
rules are the following: \rem{Aina: All of this bullets are not new in
  2018 anymore, maybe move them elsewhere? This information should not
  be lost.}
\begin{itemize}
...
\begin{center}
  \rem{TODO, this is still the old link: https://goo.gl/forms/HL6rbOywqW4sFo1D2}
  {\tt TBA}
\end{center}
...
%
A \emph{wrapper tool} is defined as any solver which calls (or is in
...
be at most 20~minutes of wall-clock time per solver/benchmark
...
purposes).
\rem{Aina: I strongly suggest that we make disagreements visible.  I
  would add a table to the bottom of the division results page, where
  we list the benchmarks we removed due to disagreements and which
  results which solvers reported. Silently dropping benchmarks on
  which solvers disagree is potentially unfair and dangerous, since
  people who want to use a solver, tend to only look at the SMT-COMP
  results and pick a solver based on this results.  They will
  certainly not go through the competition results on StarExec, in
  particular since this is quite tedious.  For users it is interesting
  (and might be actually crucial) to know if a solver disagreed with
  other solvers.  The same is true for solver developers, since they
  certainly want to know if their solver disagreed with other solvers
  on a certain benchmark.  They usually run preliminary tests with
  existing solvers beforehand, but not all solvers are available in
  the newest version.  Not to mention new solvers.}
...
\begin{itemize}
  ...
  \item Competition-wide, application track, best solver
  \item Competition-wide, unsat-core track, best solver
  \item Best newcomer
\end{itemize}
