...
\item {\bf Derived solver scoring \& recognition.} \emergencystretch.5cm A
  ...
  significantly outperforms its base solver, i.e., only if it improves its PAR2
...
A derived solver can only win a logic, track, or division under the necessary condition that it substantially outperforms its base solver. This necessary condition is to achieve \emph{at least a $10\%$ improvement on the PAR-2 score over its base solver}. Derived solvers are excluded from the biggest lead, largest contribution, and best overall ranking.
