...
\begin{description}
\item[April~12] Deadline for new benchmark contributions.
\item[May~31] Final versions of competition tools (e.g., benchmark
  scrambler) are made available.  Benchmark libraries are frozen.
\item[June~13] Deadline for first versions of solvers (for all tracks),
  including information about which tracks and divisions are being
  entered, and magic numbers for benchmark scrambling.
\item[June~27] Deadline for final versions of solvers, including
  system descriptions.
\item[June~30] Opening value of NYSE Composite Index used to compute
  random seed for competition tools.
\item[August~10-11] SMT Workshop; end of competition, presentation of
  results.
\end{description}
...
\subsection{Derived Solver Scoring} 
A derived solver can win a logic, track, or division under the necessary 
condition that it substantially outperforms its base solver. The necessary 
condition is to achieve \emph{at least a $10\%$ improvement on the PAR-2 score 
over its base solver}. Derived solvers are excluded from the best lead ranking, 
