...
\begin{itemize}
  \item {\bf Derived solver submission.} \emergencystretch.5cm Submitters of a derived solver must
  now also submit the source code of their solver 
  \rationale{Derived solver submissions are much appreciated but should be open-sourced to be useful for the community.}  % 
  \item {\bf Derived solver scoring \& recognition.} \emergencystretch.5cm A derived solver can only     
  win a division, track, or logic if it outperforms its base solver significantly, i.e.,       
  at least by a $10\%$ margin in PAR2 score. Moreover, derived solvers are excluded from competition-wide   
  recognitions. 
  \rationale{The new rule prevents that derived solvers that only marginally outperform their base solvers 
  can win awards.}    
  % 
  \item {\bf Benchmark selection.} \emergencystretch.5cm  In accordance with discussions of the SMT community, we  
   reduced the number of benchmarks to keep SMT-COMP 2026 computable in a reasonable amount of time.   
  \rationale{SMT-COMP 2025 took 9.98 CPU years to execute, yielding little buffer for errors 
  and for checking the results between the solver submission deadline and the SMT workshop.}
\end{itemize}
