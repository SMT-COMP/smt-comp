...
\item Benchmarks will likely use (a subset of) version~2.5 of the
  SMT-LIB language.  \emph{Rationale:} This is the latest version of
  the SMT-LIB language.  Its use had been announced for SMT-COMP 2016
...
\item Benchmarks that use partial bit-vector functions, such as
  \texttt{bvudiv}, may become eligible for the competition again,
  provided the semantics of these functions (according to SMT-LIB) is
  changed to agree with existing solver implementations, and the
  status of affected SMT-LIB benchmarks is updated accordingly.
  \emph{Rationale:} The semantics of these functions has been
  discussed for a long time, and it appears that a conclusion has
  finally been reached.
\item The penalty for erroneous results in the competition-wide
  scoring is changed from the number of errors to a fixed constant
  (multiplied by the weight of the respective division).
  \emph{Rationale:} The weight already takes the number of benchmarks
  in a division into account; multiplying it by the number of errors
...
unsatisfiable core is not, in fact, unsatisfiable.  The organizers
intend to check unsatisfiability by running an SMT solver on a
benchmark that asserts only those formulas in the reported core.  The
details of the checking procedure, which will be the same for all
solvers and benchmarks in the unsat-core track, are still to be
determined.
...
\item \emph{Remove inappropriate benchmarks.} The competition
  ...
  inappropriate\footnote{In 2017, this may (again) include all
    benchmarks that use partial or underspecified functions, e.g.,
    \texttt{bvudiv}, \texttt{fp.min}, unless their semantics can be
    resolved in time for the competition.}  or uninteresting for
...
(or more) sound solvers disagree on their result.  More specifically,
a solver (including a solver that was entered into the competition by
the organizers for comparison purposes) is \emph{sound} for a division
if its raw score (Section~\ref{sec:benchmark-scoring}) is of the form
$\langle 0, n, w, c\rangle$ for each benchmark in the division, i.e.,
if it did not produce any erroneous results.  Two solvers
\emph{disagree} on a benchmark if one of them reported \texttt{sat}
and the other reported \texttt{unsat}.  Only the remaining benchmarks
are used in the following computation of division scores.
...
was entered.\footnote{\emph{Rationale:} This metric purposely
  ...
  division.  The value~$-4$ balances the community's interest in
  correct (thoroughly tested) solvers against the risk of stifling
  innovation: entering a perfect (but possibly buggy) solver into a
  division has a positive expected value if the probability of a
  soundness bug is below $\frac{1}{1+4} = 20$\,\%.
