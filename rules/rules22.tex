...
The output of the solver is then fed to a proof checker, which also needs to be
provided by the solver authors (but it does not need to authored by them).
%
The input to the proof checker is the whole output of the solver including the
\texttt{unsat} reply to the \akey{check-sat} command.
%
The proof checker's output should start with a line containing either
\texttt{valid} or \texttt{invalid} to indicate that the proof was valid.
%
The rest of the output can be arbitrary (for example the proof checker may print
out statistics).
