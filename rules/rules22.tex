...
\begin{itemize}
  ...
  \item {\bf \prooftrack.}  \todo{after we iron it out.}
...
\header{Input/Output.}
The SMT-LIB language provides a command \akey{(get-proof)}, which asks
a solver to output the proof after a \akey{check-sat}
command returns \texttt{unsat}.
This proof must be an s-expression according to the SMT-LIB standard, but
the format is not specified further.
%
Solvers must respond to each command in the benchmark script with the
answers defined in the SMT-LIB format specification.  In particular,
solvers that respond \texttt{unknown} to the \akey{check-sat} command
must respond with an error to the following \akey{get-proof}
command.

The output of the solver is then fed to a proof checker, which also
needs be provided by the solver authors.  The input to the proof
checker is the whole output of the solver including the \texttt{unsat}
reply to the \akey{check-sat} command.  The proof checker should
output either \texttt{valid} or \texttt{invalid} to indicate that the
proof was valid.  \todo{Further statistics?}


\header{Result.}
The result of a solver is considered \emph{erroneous} if (i) the
response to the \akey{check-sat} command is \texttt{sat}, (ii) the
proof checker returns \akey{invalid} to the given proof.  Because the
\prooftrack is non-competitive and because there is no common
definition of a valid proof, there will be no score computed for this
track.
...
\item \todo{excluding and including benchmarks based on statuses and runs}
  For the \prooftrack and \ucoretrack, all benchmark with status
  \texttt{sat} are removed.  We further remove benchmarks with status
  \texttt{unknown} for which no solver reported them to be
  \texttt{unsat}.
  %
  For the \mvaltrack,  all benchmark with status \texttt{unsat} are removed,
...
cvc5~\cite{cvc4-smtcomp18} \hb{update reference} and veriT~\cite{verit}.
%
