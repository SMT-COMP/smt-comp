...
For the \prooftrack, solver authors should provide two new scripts
...
As describe in Section~\ref{sec:logistics}, the benchmark
...
usually start with the \akey{unsat} result of the \akey{(check-sat)}
...
proof checker should be one of \akey{valid}, \akey{invalid},
\akey{holey}, \akey{sat}, \akey{unknown}.  The answer \akey{valid}
must only be given, if the checker made sure using the proof that the
benchmark is unsat.  A checker that returns \akey{valid} to a
satisfiable benchmark is deemed to be unsound.  The answer
\akey{invalid} should be returned if the proof is errorneous or
...
checker, the answer should be \akey{holey}.  The answer \akey{sat}
should be given if the solver claimed that the benchmark is
satisfiable and \akey{unknown} should be given, if the solver answered
\akey{unknown} or if it did not give a proof or gave an error result.

After this firsts line, the proof checker can give arbitrary key/value
pairs of the form \akey{key=value} after the results.  These will be
collected in the final result table and can include information about
the number of proof steps, the number of nodes in the proof, the number
of holes, the number of different proof rules, etc.

\header{Result.}
The result of a solver is considered \emph{erroneous} if (i) the
response to the \akey{check-sat} command is \texttt{sat} and the
benchmark is unsat, (ii) the proof checker returns \akey{invalid} to
...
A proof checker is considered unsound if it returns \akey{valid} for
an invalid proof or for a proof containing unchecked steps.  In
particular, if a proof checker can be tricked to return \akey{valid}
