...
\texttt{starexec\_solver\_default}, should take the name of the benchmark file as
a single argument and run the solver on the benchmark.
%
As described in Section~\ref{sec:logistics}, the benchmark
will contains the commands \akey{(check-sat)} and \akey{(get-proof)}.
%
...
stdout.
%
A result printed to stderr is ignored.
%
...
%
The answer \texttt{valid} must only be given when the checker certifies the
...
is satisfiable, and \texttt{unknown} should be given if the solver answered
\texttt{unknown}, if it answered \texttt{unsat} but did not give a proof, or if
the solver gave an error result.
...
%
The result of a solver is considered \emph{erroneous} if (i) the
response to the \akey{check-sat} command is \texttt{sat} (as the
