...
\begin{description}
\item[Mar~??] Deadline for new benchmark contributions.
\item[May~??] Final versions of competition tools (e.g., benchmark
  scrambler) are made available.  Benchmark libraries are frozen.
\item[June~??] Deadline for first versions of solvers (for all tracks),
  including information about which tracks and divisions are being
  entered, and magic numbers for benchmark scrambling.
\item[July~?] Deadline for final versions of solvers, including
  system descriptions.
\item[July~?] Opening value of NYSE Composite Index used to compute
  random seed for competition tools.
\item[August~??] SMT Workshop; end of competition, presentation of
  results.
\end{description}
...
\begin{itemize}
  \item {\bf System description must be submitted with preliminary
      solver.} \todo{what}
%
  \rationale{\todo{reason}}
...
The deadline for submission of new benchmarks was {\bf March~15, 2022}.
%
...
\item \href{TODO}{Martin Bromberger}~-- TODO
