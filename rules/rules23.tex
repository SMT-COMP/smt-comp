...
changes in 2023 are designed to improve how logics are organized into
...
\begin{itemize}
  \item {\bf System descriptions.}  \emergencystretch.5cm The system description 
  for a participating solver must be submitted by the deadline for the preliminary 
  solver versions. In the case a system description contains not all necessary information,
  participants will be asked to submit an updated version of their 
  system description as part of the submission of the final version of their solver.
%
  \rationale{We need this time to check whether the system descriptions contain all
  necessary information and to give participants the chance to fix their system 
  descriptions if necessary.}
  
  \item {\bf System descriptions.}  \emergencystretch.5cm Some formulations 
  in the requirements for the system descriptions have been updated from 
  \emph{should} to \textbf{must} to make it clearer that these requirements 
  are mandatory and not just recommended/optional.
%
  \rationale{We did this to ensure that derived tools are more easily recognizable and 
  that it is always clear in which way wrapper tools and derived tools improve 
  on the original solvers. 
  Our changes follow the key words for use in RFCs to indicate requirement levels 
  as proposed by \url{https://www.ietf.org/rfc/rfc2119.txt}.}

  \item {\bf \mvaltrack.}  \emergencystretch.5cm Last year's
    competition introduced the experimental divisions QF\_FPArith
    (excluding the logics QF\_ABVFP, QF\_AUFBVFP, and QF\_ABVFPLRA).
  %
  This year these divisions will no longer be experimental.
  %
  In addition, we add this year the new experimental division for
  arrays, non-linear arithmetic and for datatypes.  These include the divisions
  QF\_NonLinearIntArith, QF\_NonLinearRealArith,
  QF\_Equality+NonLinearArithm,
  and QF\_Datatypes.
%
  \rationale{Last year's divisions ran successfully, so there
    ...
    model standardization. We exclude logics containing arrays since
    there is no standardized way of expressing array model values and
    we do not want to add too many new logics.}

\item {\bf \prooftrack.}
  Last year we added this as experimental track. We will continue with
  this track in more or less the same format.  We will reduce the
  number of benchmarks, though.
%  
  \rationale{We received several submissions that showed that there is
    interest in a proof track.  

    We are still far away from a common proof format, so changing the
    track to a competition track or making it non-experimental is not
    feasible at this time.

    The number of benchmarks and the size of the produced proofs
    turned out to be a major stress to the StarExec infrastructure,
    while not giving only little insight.  Therefore, we will reduce
    the number of benchmarks in this track.
  }

\item {\bf Wrapper Script for \prooftrack.}
   The wrapper script is changed to remove very large proofs and
   to cut very large error/debug output.
%
   \rationale{Last Year, the \prooftrack produced 800~GB of output, most
   of that stemming from a few benchmark/solver pairs. These cannot be
   removed without losing the whole proof track job.   It also makes
   downloading and sharing the result much harder.}
   
\item {\bf Scoring.}  \emergencystretch.5cm We adjusted the wall-clock and 
CPU time scores so only the benchmarks that are correctly solved within the 
time limit $T$ are counted.
%
  \rationale{We noticed that in the previous scoring unintended results 
  did occur because timeouts were scored worse than aborts and could 
  overshadow the times of correctly solved problems. For instance, 
  solver A that never aborts could lose to solver B that frequently aborts 
  although A solved exactly the same problems B could solve and A was always 
  faster on the solved problems than B.}
\end{itemize}
...
%
This year all divisions with non-linear arithmetic, arrays, and data
types are experimental divisions.
%
...
is the (real-valued) wall-clock time in seconds until the process returned the last correct result for a \akey{check-sat} command within the time limit;
this means especially that $w = 0$ is zero if no (correct) result was returned for the first \akey{check-sat} command within the time limit.
...
is the (real-valued) CPU time in seconds until the process returned the last correct result for a \akey{check-sat} command within the time limit;
this means especially that $c = 0$ is zero if no (correct) result was returned for the first \akey{check-sat} command within the time limit.
